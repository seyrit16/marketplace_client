import React, {useState, useEffect, useRef} from 'react';
import {User, Mail, Building, CreditCard, Trash2, CheckCircle, Lock, EyeOff, Eye} from 'lucide-react';
import "./SellerProfile.style.scss"
import type {EmailUpdateErrors, PasswordUpdateErrors} from "../../../models/User.model.ts";

// Mock seller data based on the interface
const mockSeller = {
    email: "seller@example.com",
    isActive: true,
    isLocked: false,
    sellerProfile: {
        fullCompanyName: "ООО Пример",
        shortCompanyName: "Пример",
        description: "ООО Пример — ведущий поставщик качественной одежды. Мы работаем на рынке более 10 лет и зарекомендовали себя как надежный партнер среди клиентов по всей России. Наша миссия — обеспечивать клиентов лучшими продуктами по доступным ценам. Контроль качества и клиентоориентированность — наш приоритет."
    },
    personDetail: {
        surname: "Иванов",
        name: "Иван",
        patronimyc: "Иванович",
        phoneNumber: "+7900123456"
    },
    paymentDetail: {
        bankAccountNumber: "12345678901234567890",
        bankName: "Сбербанк",
        bic: "123456789",
        accountHolderName: "Иванов Иван Иванович",
        inn: "1234567890"
    }
};

type ProfileSection = 'overview' | 'company' | 'personal' | 'payment' | 'delete' | 'email' | 'password';

interface CompanyFormData {
    fullCompanyName: string;
    shortCompanyName: string;
    description: string;
}

interface PersonalFormData {
    surname: string;
    name: string;
    patronimyc: string;
    phoneNumber: string;
}

interface PaymentFormData {
    bankAccountNumber: string;
    bankName: string;
    bic: string;
    accountHolderName: string;
    inn: string;
}

interface FormErrors {
    company?: Partial<CompanyFormData>;
    personal?: Partial<PersonalFormData>;
    payment?: Partial<PaymentFormData>;
    email?: EmailUpdateErrors;
    password?: PasswordUpdateErrors;
}

const SellerProfile: React.FC = () => {
    const [sellerData, setSellerData] = useState(mockSeller);
    const [activeSection, setActiveSection] = useState<ProfileSection>('overview');
    const [errors, setErrors] = useState<FormErrors>({});
    const [successMessage, setSuccessMessage] = useState<string>('');
    const [isLoading, setIsLoading] = useState(false);


    // Code input states
    const [emailCodeInputs, setEmailCodeInputs] = useState<string[]>(Array(6).fill(''));
    const [passwordCodeInputs, setPasswordCodeInputs] = useState<string[]>(Array(6).fill(''));

    // Refs for autofocus
    const emailInputRefs = useRef<(HTMLInputElement | null)[]>([]);
    const passwordInputRefs = useRef<(HTMLInputElement | null)[]>([]);

    // Form states
    const [companyForm, setCompanyForm] = useState<CompanyFormData>({
        fullCompanyName: sellerData.sellerProfile.fullCompanyName,
        shortCompanyName: sellerData.sellerProfile.shortCompanyName,
        description: sellerData.sellerProfile.description,
    });

    const [personalForm, setPersonalForm] = useState<PersonalFormData>({
        surname: sellerData.personDetail.surname,
        name: sellerData.personDetail.name,
        patronimyc: sellerData.personDetail.patronimyc,
        phoneNumber: sellerData.personDetail.phoneNumber,
    });

    const [paymentForm, setPaymentForm] = useState<PaymentFormData>({
        bankAccountNumber: sellerData.paymentDetail.bankAccountNumber,
        bankName: sellerData.paymentDetail.bankName,
        bic: sellerData.paymentDetail.bic,
        accountHolderName: sellerData.paymentDetail.accountHolderName,
        inn: sellerData.paymentDetail.inn,
    });


    // Email update form state
    const [emailForm, setEmailForm] = useState({
        newEmail: '',
        verificationCode: '',
        step: 1,
    });

    // Password update form state
    const [passwordForm, setPasswordForm] = useState({
        currentPassword: '',
        newPassword: '',
        verificationCode: '',
        step: 1,
        showCurrentPassword: false,
        showNewPassword: false,
    });

    const [deleteConfirmation, setDeleteConfirmation] = useState('');

    // Update forms when seller data changes
    useEffect(() => {
        setCompanyForm({
            fullCompanyName: sellerData.sellerProfile.fullCompanyName,
            shortCompanyName: sellerData.sellerProfile.shortCompanyName,
            description: sellerData.sellerProfile.description,
        });
        setPersonalForm({
            surname: sellerData.personDetail.surname,
            name: sellerData.personDetail.name,
            patronimyc: sellerData.personDetail.patronimyc,
            phoneNumber: sellerData.personDetail.phoneNumber,
        });
        setPaymentForm({
            bankAccountNumber: sellerData.paymentDetail.bankAccountNumber,
            bankName: sellerData.paymentDetail.bankName,
            bic: sellerData.paymentDetail.bic,
            accountHolderName: sellerData.paymentDetail.accountHolderName,
            inn: sellerData.paymentDetail.inn,
        });
    }, [sellerData]);

    // Clear messages after timeout
    useEffect(() => {
        if (successMessage) {
            const timer = setTimeout(() => setSuccessMessage(''), 3000);
            return () => clearTimeout(timer);
        }
    }, [successMessage]);

    // Validation functions
    const validateCompanyForm = (): boolean => {
        const companyErrors: Partial<CompanyFormData> = {};

        if (!companyForm.fullCompanyName.trim()) {
            companyErrors.fullCompanyName = 'Полное название компании обязательно';
        }
        if (!companyForm.shortCompanyName.trim()) {
            companyErrors.shortCompanyName = 'Краткое название компании обязательно';
        }
        if (!companyForm.description.trim()) {
            companyErrors.description = 'Описание деятельности обязательно';
        }

        setErrors(prev => ({...prev, company: companyErrors}));
        return Object.keys(companyErrors).length === 0;
    };

    const validatePersonalForm = (): boolean => {
        const personalErrors: Partial<PersonalFormData> = {};

        if (!personalForm.name.trim()) {
            personalErrors.name = 'Имя обязательно';
        }
        if (!personalForm.surname.trim()) {
            personalErrors.surname = 'Фамилия обязательна';
        }
        if (personalForm.phoneNumber && !/^\+?[1-9]\d{1,14}$/.test(personalForm.phoneNumber)) {
            personalErrors.phoneNumber = 'Некорректный номер телефона';
        }

        setErrors(prev => ({...prev, personal: personalErrors}));
        return Object.keys(personalErrors).length === 0;
    };

    const validatePaymentForm = (): boolean => {
        const paymentErrors: Partial<PaymentFormData> = {};

        if (!paymentForm.bankAccountNumber.trim() || !/^\d{20}$/.test(paymentForm.bankAccountNumber)) {
            paymentErrors.bankAccountNumber = 'Номер счета должен содержать 20 цифр';
        }
        if (!paymentForm.bankName.trim()) {
            paymentErrors.bankName = 'Название банка обязательно';
        }
        if (!paymentForm.bic.trim() || !/^\d{9}$/.test(paymentForm.bic)) {
            paymentErrors.bic = 'БИК должен содержать 9 цифр';
        }
        if (!paymentForm.accountHolderName.trim()) {
            paymentErrors.accountHolderName = 'Имя владельца счета обязательно';
        }
        if (!paymentForm.inn.trim() || !/^\d{10}$/.test(paymentForm.inn)) {
            paymentErrors.inn = 'ИНН должен содержать 10 цифр';
        }

        setErrors(prev => ({...prev, payment: paymentErrors}));
        return Object.keys(paymentErrors).length === 0;
    };

    const validateEmail = (email: string) => {
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        return emailRegex.test(email) ? '' : 'Некорректный email адрес';
    };

    const validatePassword = (password: string) => {
        const passwordRegex = /^(?=.*[A-Za-zА-Яа-яЁё])(?=.*\d)[A-Za-zА-Яа-яЁё\d]{8,}$/;
        return passwordRegex.test(password)
            ? ''
            : 'Пароль должен содержать минимум 8 символов, включая буквы и цифры';
    };

    // Submit handlers
    const handleCompanyUpdate = async () => {
        if (!validateCompanyForm()) return;

        setIsLoading(true);
        try {
            // Simulate API call
            await new Promise(resolve => setTimeout(resolve, 1000));

            setSellerData(prev => ({
                ...prev,
                sellerProfile: {
                    ...prev.sellerProfile,
                    ...companyForm
                }
            }));

            setSuccessMessage('Информация о компании успешно обновлена');
            setActiveSection('overview');
        } catch (error) {
            console.error('Ошибка обновления информации о компании:', error);
        } finally {
            setIsLoading(false);
        }
    };

    const handlePersonalUpdate = async () => {
        if (!validatePersonalForm()) return;

        setIsLoading(true);
        try {
            // Simulate API call
            await new Promise(resolve => setTimeout(resolve, 1000));

            setSellerData(prev => ({
                ...prev,
                personDetail: {
                    ...prev.personDetail,
                    ...personalForm
                }
            }));

            setSuccessMessage('Контактные данные успешно обновлены');
            setActiveSection('overview');
        } catch (error) {
            console.error('Ошибка обновления контактных данных:', error);
        } finally {
            setIsLoading(false);
        }
    };

    const handlePaymentUpdate = async () => {
        if (!validatePaymentForm()) return;

        setIsLoading(true);
        try {
            // Simulate API call
            await new Promise(resolve => setTimeout(resolve, 1000));

            setSellerData(prev => ({
                ...prev,
                paymentDetail: {
                    ...prev.paymentDetail,
                    ...paymentForm
                }
            }));

            setSuccessMessage('Платежные реквизиты успешно обновлены');
            setActiveSection('overview');
        } catch (error) {
            console.error('Ошибка обновления платежных реквизитов:', error);
        } finally {
            setIsLoading(false);
        }
    };

    const handleDeleteProfile = async () => {
        if (deleteConfirmation !== 'УДАЛИТЬ') {
            alert('Введите "УДАЛИТЬ" для подтверждения');
            return;
        }

        setIsLoading(true);
        try {
            await new Promise(resolve => setTimeout(resolve, 1000));

            setSuccessMessage('Профиль продавца успешно удален');
            setActiveSection('overview');
        } catch (error) {
            console.error('Ошибка удаления профиля:', error);
        } finally {
            setIsLoading(false);
        }
    };

    // Code input handlers
    const handleCodeChange = (
        index: number,
        value: string,
        codeInputs: string[],
        setCodeInputs: React.Dispatch<React.SetStateAction<string[]>>,
        inputRefs: React.MutableRefObject<(HTMLInputElement | null)[]>
    ) => {
        if (/^[0-9]?$/.test(value)) {
            const newCodeInputs = [...codeInputs];
            newCodeInputs[index] = value;
            setCodeInputs(newCodeInputs);

            if (value && index < 5) {
                inputRefs.current[index + 1]?.focus();
            }
        }
    };

    const handleCodeKeyDown = (
        index: number,
        e: React.KeyboardEvent<HTMLInputElement>,
        codeInputs: string[],
        inputRefs: React.MutableRefObject<(HTMLInputElement | null)[]>
    ) => {
        if (e.key === 'Backspace' && !codeInputs[index] && index > 0) {
            inputRefs.current[index - 1]?.focus();
        }
    };


    const handleEmailUpdate = async () => {
        if (emailForm.step === 1) {
            const emailError = validateEmail(emailForm.newEmail);
            if (emailError) {
                setErrors(prev => ({...prev, email: {newEmail: emailError}}));
                return;
            }

            try {
                // todo добавить отправку кода
                //await sendEmailVerificationCode(emailForm.newEmail);
                setEmailForm(prev => ({...prev, step: 2}));
                setErrors(prev => ({...prev, email: {}}));
            } catch (error) {
                console.error('<UNK> <UNK> <UNK>:', error);
                setErrors(prev => ({...prev, email: {newEmail: 'Ошибка отправки кода'}}));
            }
        } else {
            const code = emailCodeInputs.join('');
            if (code.length !== 6) {
                setErrors(prev => ({...prev, email: {verificationCode: 'Введите полный код'}}));
                return;
            }

            try {
                //await updateUserEmail(emailForm.newEmail, code);
                setSuccessMessage('Email успешно обновлен');
                setEmailForm({newEmail: '', verificationCode: '', step: 1});
                setEmailCodeInputs(Array(6).fill(''));
                setActiveSection('overview');
            } catch (error) {
                console.error('<UNK> <UNK> <UNK>:', error);
                setErrors(prev => ({...prev, email: {verificationCode: 'Неверный код'}}));
            }
        }
    };


    const handlePasswordUpdate = async () => {
        if (passwordForm.step === 1) {
            const passwordError = validatePassword(passwordForm.newPassword);
            if (passwordError) {
                setErrors(prev => ({...prev, password: {newPassword: passwordError}}));
                return;
            }

            try {
                // todo добавить отправку кода
                //await sendPasswordResetCode();
                setPasswordForm(prev => ({...prev, step: 2}));
                setErrors(prev => ({...prev, password: {}}));
            } catch (error) {
                console.error('<UNK> <UNK> <UNK>:', error);
                setErrors(prev => ({...prev, password: {currentPassword: 'Ошибка отправки кода'}}));
            }
        } else {
            const code = passwordCodeInputs.join('');
            if (code.length !== 6) {
                setErrors(prev => ({...prev, password: {verificationCode: 'Введите полный код'}}));
                return;
            }

            try {
                //await updateUserPassword(passwordForm.currentPassword, passwordForm.newPassword, code);
                setSuccessMessage('Пароль успешно обновлен');
                setPasswordForm({
                    currentPassword: '',
                    newPassword: '',
                    verificationCode: '',
                    step: 1,
                    showCurrentPassword: false,
                    showNewPassword: false,
                });
                setPasswordCodeInputs(Array(6).fill(''));
                setActiveSection('overview');
            } catch (error) {
                console.error('<UNK> <UNK> <UNK>:', error);
                setErrors(prev => ({...prev, password: {verificationCode: 'Неверный код'}}));
            }
        }
    };

    return (
        <div className="min-h-screen bg-gray-50 py-8">
            <div className="seller-profile max-w-6xl mx-auto px-4">
                {successMessage && (
                    <div
                        className="success-message mb-6 p-4 bg-green-100 border border-green-400 text-green-700 rounded-lg flex items-center">
                        <CheckCircle size={20} className="mr-2"/>
                        {successMessage}
                    </div>
                )}

                <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                    <div className="profile-wrapper flex">
                        {/* Sidebar */}
                        <div className="sidebar w-1/4 bg-gray-100 border-r">
                            <div className="seller-header p-6 border-b">
                                <div className="company-info flex items-center space-x-3">
                                    <div
                                        className="avatar w-12 h-12 bg-blue-500 rounded-full flex items-center justify-center">
                                        <Building size={24} className="text-white"/>
                                    </div>
                                    <div className={"text-info"}>
                                        <h2 className="font-semibold text-lg">{sellerData.sellerProfile.shortCompanyName}</h2>
                                        <p className="text-sm text-gray-600">{sellerData.email}</p>
                                        <div className="status flex items-center mt-1">
                                            <div
                                                className={`w-2 h-2 rounded-full mr-2 ${sellerData.isActive ? 'bg-green-500' : 'bg-red-500'}`}></div>
                                            <span className="text-xs text-gray-500">
                                                {sellerData.isActive ? 'Активен' : 'Неактивен'}
                                                {sellerData.isLocked && ' (Заблокирован)'}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <nav className="nav p-4 space-y-2">
                                <button
                                    onClick={() => setActiveSection('overview')}
                                    className={`nav-item w-full flex items-center space-x-3 px-3 py-2 rounded-lg text-left transition-colors ${
                                        activeSection === 'overview' ? 'active' : ''
                                    }`}
                                >
                                    <User size={20}/>
                                    <span>Обзор</span>
                                </button>
                                <button
                                    className={`nav-item ${activeSection === 'email' ? 'active' : ''}`}
                                    onClick={() => setActiveSection('email')}
                                >
                                    <Mail size={20}/>
                                    <span>Изменить Email</span>
                                </button>
                                <button
                                    className={`nav-item ${activeSection === 'password' ? 'active' : ''}`}
                                    onClick={() => setActiveSection('password')}
                                >
                                    <Lock size={20}/>
                                    <span>Изменить пароль</span>
                                </button>
                                <button
                                    onClick={() => setActiveSection('company')}
                                    className={`nav-item w-full flex items-center space-x-3 px-3 py-2 rounded-lg text-left transition-colors ${
                                        activeSection === 'company' ? 'active' : ''
                                    }`}
                                >
                                    <Building size={20}/>
                                    <span>Информация о компании</span>
                                </button>
                                <button
                                    onClick={() => setActiveSection('personal')}
                                    className={`nav-item w-full flex items-center space-x-3 px-3 py-2 rounded-lg text-left transition-colors ${
                                        activeSection === 'personal' ? 'active' : ''
                                    }`}
                                >
                                    <Mail size={20}/>
                                    <span>Контактные данные</span>
                                </button>
                                <button
                                    onClick={() => setActiveSection('payment')}
                                    className={`nav-item w-full flex items-center space-x-3 px-3 py-2 rounded-lg text-left transition-colors ${
                                        activeSection === 'payment' ? 'active' : ''
                                    }`}
                                >
                                    <CreditCard size={20}/>
                                    <span>Платежные реквизиты</span>
                                </button>
                                <button
                                    onClick={() => setActiveSection('delete')}
                                    className={`nav-item w-full flex items-center space-x-3 px-3 py-2 rounded-lg text-left transition-colors ${
                                        activeSection === 'delete' ? 'delete' : ''
                                    }`}
                                >
                                    <Trash2 size={20}/>
                                    <span>Удаление профиля</span>
                                </button>
                            </nav>
                        </div>

                        {/* Main Content */}
                        <div className=" seller-overview flex-1 p-8">
                            {activeSection === 'overview' && (
                                <div>
                                    <h3 className="text-2xl font-bold mb-6">Обзор профиля</h3>
                                    <div className="info-grid grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="info-block bg-gray-50 p-6 rounded-lg">
                                            <h4 className="font-semibold text-lg mb-3">Информация о компании</h4>
                                            <div className="space-y-2">
                                                <p><strong>Полное
                                                    название:</strong> {sellerData.sellerProfile.fullCompanyName}</p>
                                                <p><strong>Краткое
                                                    название:</strong> {sellerData.sellerProfile.shortCompanyName}</p>
                                                <p><strong>Описание:</strong></p>
                                                <p className="text-sm text-gray-600">{sellerData.sellerProfile.description}</p>
                                            </div>
                                        </div>

                                        <div className="info-block bg-gray-50 p-6 rounded-lg">
                                            <h4 className="font-semibold text-lg mb-3">Контактная информация</h4>
                                            <div className="space-y-2">
                                                <p>
                                                    <strong>ФИО:</strong> {sellerData.personDetail.surname} {sellerData.personDetail.name} {sellerData.personDetail.patronimyc}
                                                </p>
                                                <p><strong>Телефон:</strong> {sellerData.personDetail.phoneNumber}</p>
                                                <p><strong>Email:</strong> {sellerData.email}</p>
                                            </div>
                                        </div>

                                        <div className="info-block full-width bg-gray-50 p-6 rounded-lg md:col-span-2">
                                            <h4 className="font-semibold text-lg mb-3">Платежные реквизиты</h4>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <p><strong>Номер
                                                    счета:</strong> {sellerData.paymentDetail.bankAccountNumber}</p>
                                                <p><strong>Банк:</strong> {sellerData.paymentDetail.bankName}</p>
                                                <p><strong>БИК:</strong> {sellerData.paymentDetail.bic}</p>
                                                <p><strong>Владелец
                                                    счета:</strong> {sellerData.paymentDetail.accountHolderName}</p>
                                                <p><strong>ИНН:</strong> {sellerData.paymentDetail.inn}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            )}

                            {activeSection === 'email' && (
                                <div className="email-section">
                                    <h3>Изменить Email</h3>
                                    {emailForm.step === 1 ? (
                                        <form className="email-form">
                                            <div className="form-group">
                                                <label>Текущий email</label>
                                                <input type="email" value={sellerData.email} disabled/>
                                            </div>
                                            <div className="form-group">
                                                <label>Новый email *</label>
                                                <input
                                                    type="email"
                                                    value={emailForm.newEmail}
                                                    onChange={(e) => setEmailForm(prev => ({
                                                        ...prev,
                                                        newEmail: e.target.value
                                                    }))}
                                                    className={errors.email?.newEmail ? 'error' : ''}
                                                />
                                                {errors.email?.newEmail &&
                                                    <span className="error-text">{errors.email.newEmail}</span>}
                                            </div>
                                            <div className={"form-actions"}>
                                                <button type="button" onClick={handleEmailUpdate} className="submit-button">
                                                    Отправить код подтверждения
                                                </button>
                                            </div>

                                        </form>
                                    ) : (
                                        <div className="verification-form">
                                            <p>Код подтверждения отправлен на {sellerData.email}</p>
                                            <div className="code-inputs">
                                                {emailCodeInputs.map((value, index) => (
                                                    <input
                                                        key={index}
                                                        type="text"
                                                        maxLength={1}
                                                        value={value}
                                                        onChange={(e) => handleCodeChange(index, e.target.value, emailCodeInputs, setEmailCodeInputs, emailInputRefs)}
                                                        onKeyDown={(e) => handleCodeKeyDown(index, e, emailCodeInputs, emailInputRefs)}
                                                        ref={(el) => {
                                                            emailInputRefs.current[index] = el;
                                                        }}
                                                        className="code-input"
                                                    />
                                                ))}
                                            </div>
                                            {errors.email?.verificationCode &&
                                                <span className="error-text">{errors.email.verificationCode}</span>}
                                            <div className="form-actions">
                                                <button
                                                    type="button"
                                                    onClick={() => setEmailForm(prev => ({...prev, step: 1}))}
                                                    className="secondary-button"
                                                >
                                                    Назад
                                                </button>
                                                <button type="button" onClick={handleEmailUpdate}
                                                        className="submit-button">
                                                    Подтвердить
                                                </button>
                                            </div>
                                        </div>
                                    )}
                                </div>
                            )}

                            {activeSection === 'password' && (
                                <div className="password-section">
                                    <h3>Изменить пароль</h3>
                                    {passwordForm.step === 1 ? (
                                        <form className="password-form">

                                            <div className="form-group">
                                                <label>Новый пароль *</label>
                                                <div className="password-input">
                                                    <input
                                                        type={passwordForm.showNewPassword ? 'text' : 'password'}
                                                        value={passwordForm.newPassword}
                                                        onChange={(e) => setPasswordForm(prev => ({
                                                            ...prev,
                                                            newPassword: e.target.value
                                                        }))}
                                                        className={errors.password?.newPassword ? 'error' : ''}
                                                    />
                                                    <button
                                                        type="button"
                                                        onClick={() => setPasswordForm(prev => ({
                                                            ...prev,
                                                            showNewPassword: !prev.showNewPassword
                                                        }))}
                                                        className="password-toggle"
                                                    >
                                                        {passwordForm.showNewPassword ? <EyeOff size={20}/> :
                                                            <Eye size={20}/>}
                                                    </button>
                                                </div>
                                                {errors.password?.newPassword &&
                                                    <span className="error-text">{errors.password.newPassword}</span>}
                                            </div>

                                            <div className={"form-actions"}>
                                                <button type="button" onClick={handlePasswordUpdate}
                                                        className="submit-button">
                                                    Отправить код подтверждения
                                                </button>
                                            </div>

                                        </form>
                                    ) : (
                                        <div className="verification-form">
                                            <p>Код подтверждения отправлен на ваш email</p>
                                            <div className="code-inputs">
                                                {passwordCodeInputs.map((value, index) => (
                                                    <input
                                                        key={index}
                                                        type="text"
                                                        maxLength={1}
                                                        value={value}
                                                        onChange={(e) => handleCodeChange(index, e.target.value, passwordCodeInputs, setPasswordCodeInputs, passwordInputRefs)}
                                                        onKeyDown={(e) => handleCodeKeyDown(index, e, passwordCodeInputs, passwordInputRefs)}
                                                        ref={(el) => {
                                                            passwordInputRefs.current[index] = el;
                                                        }}
                                                        className="code-input"
                                                    />
                                                ))}
                                            </div>
                                            {errors.password?.verificationCode &&
                                                <span className="error-text">{errors.password.verificationCode}</span>}
                                            <div className="form-actions">
                                                <button
                                                    type="button"
                                                    onClick={() => setPasswordForm(prev => ({...prev, step: 1}))}
                                                    className="secondary-button"
                                                >
                                                    Назад
                                                </button>
                                                <button type="button" onClick={handlePasswordUpdate}
                                                        className="submit-button">
                                                    Подтвердить
                                                </button>
                                            </div>
                                        </div>
                                    )}
                                </div>
                            )}

                            {activeSection === 'company' && (
                                <div className={"company-section"}>
                                    <h3 className="text-2xl font-bold mb-6">Информация о компании</h3>
                                    <div className="form-wrapper max-w-2xl">
                                        <div className="space-y-4form-group">
                                            <div className="form-group">
                                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                                    Полное название компании *
                                                </label>
                                                <input
                                                    type="text"
                                                    value={companyForm.fullCompanyName}
                                                    onChange={(e) => setCompanyForm(prev => ({
                                                        ...prev,
                                                        fullCompanyName: e.target.value
                                                    }))}
                                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                                                        errors.company?.fullCompanyName ? 'border-red-500 error' : 'border-gray-300'
                                                    }`}
                                                />
                                                {errors.company?.fullCompanyName && (
                                                    <p className=" error-text text-red-500 text-sm mt-1">{errors.company.fullCompanyName}</p>
                                                )}
                                            </div>

                                            <div className="form-group">
                                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                                    Краткое название компании *
                                                </label>
                                                <input
                                                    type="text"
                                                    value={companyForm.shortCompanyName}
                                                    onChange={(e) => setCompanyForm(prev => ({
                                                        ...prev,
                                                        shortCompanyName: e.target.value
                                                    }))}
                                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                                                        errors.company?.shortCompanyName ? 'border-red-500 error' : 'border-gray-300'
                                                    }`}
                                                />
                                                {errors.company?.shortCompanyName && (
                                                    <p className="text-red-500 text-sm mt-1 error ">{errors.company.shortCompanyName}</p>
                                                )}
                                            </div>

                                            <div className="form-group">
                                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                                    Описание деятельности *
                                                </label>
                                                <textarea
                                                    value={companyForm.description}
                                                    onChange={(e) => setCompanyForm(prev => ({
                                                        ...prev,
                                                        description: e.target.value
                                                    }))}
                                                    rows={4}
                                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                                                        errors.company?.description ? 'border-red-500 error' : 'border-gray-300'
                                                    }`}
                                                />
                                                {errors.company?.description && (
                                                    <p className="text-red-500 text-sm mt-1 error">{errors.company.description}</p>
                                                )}
                                            </div>

                                            <div className="form-actions flex space-x-4 pt-4">
                                                <button
                                                    onClick={handleCompanyUpdate}
                                                    disabled={isLoading}
                                                    className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed submit-button"
                                                >
                                                    {isLoading ? 'Сохранение...' : 'Сохранить изменения'}
                                                </button>
                                                <button
                                                    onClick={() => setActiveSection('overview')}
                                                    className="px-6 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 cancel-button"
                                                >
                                                    Отмена
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            )}

                            {activeSection === 'personal' && (
                                <div className="personal-section">
                                    <h3 className="text-2xl font-bold mb-6">Контактные данные</h3>
                                    <div className="max-w-2xl form-wrapper">
                                        <div className="space-y-4">
                                            <div className={"form-group"}>
                                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                                    Фамилия *
                                                </label>
                                                <input
                                                    type="text"
                                                    value={personalForm.surname}
                                                    onChange={(e) => setPersonalForm(prev => ({
                                                        ...prev,
                                                        surname: e.target.value
                                                    }))}
                                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                                                        errors.personal?.surname ? 'border-red-500 error' : 'border-gray-300'
                                                    }`}
                                                />
                                                {errors.personal?.surname && (
                                                    <p className="text-red-500 text-sm mt-1 error-text">{errors.personal.surname}</p>
                                                )}
                                            </div>

                                            <div className={"form-group"}>
                                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                                    Имя *
                                                </label>
                                                <input
                                                    type="text"
                                                    value={personalForm.name}
                                                    onChange={(e) => setPersonalForm(prev => ({
                                                        ...prev,
                                                        name: e.target.value
                                                    }))}
                                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                                                        errors.personal?.name ? 'border-red-500 error' : 'border-gray-300'
                                                    }`}
                                                />
                                                {errors.personal?.name && (
                                                    <p className="text-red-500 text-sm mt-1 error-text">{errors.personal.name}</p>
                                                )}
                                            </div>

                                            <div className={"form-group"}>
                                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                                    Отчество
                                                </label>
                                                <input
                                                    type="text"
                                                    value={personalForm.patronimyc}
                                                    onChange={(e) => setPersonalForm(prev => ({
                                                        ...prev,
                                                        patronimyc: e.target.value
                                                    }))}
                                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                />
                                            </div>

                                            <div className={"form-group"}>
                                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                                    Номер телефона
                                                </label>
                                                <input
                                                    type="tel"
                                                    value={personalForm.phoneNumber}
                                                    onChange={(e) => setPersonalForm(prev => ({
                                                        ...prev,
                                                        phoneNumber: e.target.value
                                                    }))}
                                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                                                        errors.personal?.phoneNumber ? 'border-red-500 error' : 'border-gray-300'
                                                    }`}
                                                />
                                                {errors.personal?.phoneNumber && (
                                                    <p className="text-red-500 text-sm mt-1 error-text">{errors.personal.phoneNumber}</p>
                                                )}
                                            </div>

                                            <div className="flex space-x-4 pt-4 form-actions">
                                                <button
                                                    onClick={handlePersonalUpdate}
                                                    disabled={isLoading}
                                                    className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed submit-button"
                                                >
                                                    {isLoading ? 'Сохранение...' : 'Сохранить изменения'}
                                                </button>
                                                <button
                                                    onClick={() => setActiveSection('overview')}
                                                    className="px-6 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 cancel-button"
                                                >
                                                    Отмена
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            )}

                            {activeSection === 'payment' && (
                                <div className="payment-section">
                                    <h3 className="text-2xl font-bold mb-6">Платежные реквизиты</h3>
                                    <div className="max-w-2xl form-wrapper">
                                        <div className="space-y-4">
                                            <div className={"form-group"}>
                                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                                    Номер банковского счета *
                                                </label>
                                                <input
                                                    type="text"
                                                    value={paymentForm.bankAccountNumber}
                                                    onChange={(e) => setPaymentForm(prev => ({
                                                        ...prev,
                                                        bankAccountNumber: e.target.value.replace(/\D/g, '')
                                                    }))}
                                                    maxLength={20}
                                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                                                        errors.payment?.bankAccountNumber ? 'border-red-500 errors ' : 'border-gray-300'
                                                    }`}
                                                />
                                                {errors.payment?.bankAccountNumber && (
                                                    <p className="text-red-500 text-sm mt-1 error-text">{errors.payment.bankAccountNumber}</p>
                                                )}
                                            </div>

                                            <div className={"form-group"}>
                                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                                    Название банка *
                                                </label>
                                                <input
                                                    type="text"
                                                    value={paymentForm.bankName}
                                                    onChange={(e) => setPaymentForm(prev => ({
                                                        ...prev,
                                                        bankName: e.target.value
                                                    }))}
                                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                                                        errors.payment?.bankName ? 'border-red-500 errors ' : 'border-gray-300'
                                                    }`}
                                                />
                                                {errors.payment?.bankName && (
                                                    <p className="text-red-500 text-sm mt-1 error-text">{errors.payment.bankName}</p>
                                                )}
                                            </div>

                                            <div className={"form-group"}>
                                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                                    БИК *
                                                </label>
                                                <input
                                                    type="text"
                                                    value={paymentForm.bic}
                                                    onChange={(e) => setPaymentForm(prev => ({
                                                        ...prev,
                                                        bic: e.target.value.replace(/\D/g, '')
                                                    }))}
                                                    maxLength={9}
                                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                                                        errors.payment?.bic ? 'border-red-500 errors ' : 'border-gray-300'
                                                    }`}
                                                />
                                                {errors.payment?.bic && (
                                                    <p className="text-red-500 text-sm mt-1 error-text">{errors.payment.bic}</p>
                                                )}
                                            </div>

                                            <div className={"form-group"}>
                                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                                    Имя владельца счета *
                                                </label>
                                                <input
                                                    type="text"
                                                    value={paymentForm.accountHolderName}
                                                    onChange={(e) => setPaymentForm(prev => ({
                                                        ...prev,
                                                        accountHolderName: e.target.value
                                                    }))}
                                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                                                        errors.payment?.accountHolderName ? 'border-red-500 errors ' : 'border-gray-300'
                                                    }`}
                                                />
                                                {errors.payment?.accountHolderName && (
                                                    <p className="text-red-500 text-sm mt-1 error-text">{errors.payment.accountHolderName}</p>
                                                )}
                                            </div>

                                            <div className={"form-group"}>
                                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                                    ИНН *
                                                </label>
                                                <input
                                                    type="text"
                                                    value={paymentForm.inn}
                                                    onChange={(e) => setPaymentForm(prev => ({
                                                        ...prev,
                                                        inn: e.target.value.replace(/\D/g, '')
                                                    }))}
                                                    maxLength={10}
                                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                                                        errors.payment?.inn ? 'border-red-500 errors ' : 'border-gray-300'
                                                    }`}
                                                />
                                                {errors.payment?.inn && (
                                                    <p className="text-red-500 text-sm mt-1 error-text">{errors.payment.inn}</p>
                                                )}
                                            </div>

                                            <div className="flex space-x-4 pt-4 form-actions">
                                                <button
                                                    onClick={handlePaymentUpdate}
                                                    disabled={isLoading}
                                                    className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed submit-button"
                                                >
                                                    {isLoading ? 'Сохранение...' : 'Сохранить изменения'}
                                                </button>
                                                <button
                                                    onClick={() => setActiveSection('overview')}
                                                    className="px-6 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 cancel-button"
                                                >
                                                    Отмена
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                                </div>)}

                            {activeSection === 'delete' && (
                                <div className="delete-section">
                                    <h3 className="text-2xl font-bold mb-6 text-red-600">Удаление профиля продавца</h3>
                                    <div className="max-w-2xl form-wrapper">
                                        <div className="bg-red-50 border border-red-200 rounded-lg p-6 mb-6">
                                            <div className="flex items-start">
                                                <Trash2 className="text-red-500 mr-3 mt-1" size={24}/>
                                                <div className={"form-group"}>
                                                    <h4 className="text-lg font-semibold text-red-800 mb-2 disclaimer">
                                                        Внимание!
                                                    </h4>
                                                    <div className="text-red-700 space-y-2 ">
                                                        <p>При удалении профиля продавца будут удалены:</p>
                                                        <ul className="list-disc list-inside ml-4 space-y-1">
                                                            <li>Все данные о компании и контактная информация</li>
                                                            <li>Платежные реквизиты и банковские данные</li>
                                                            <li>Все товары и их описания</li>
                                                            <li>Доступ к личному кабинету</li>
                                                        </ul>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        <div className="space-y-6">
                                            <div className="form-group">
                                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                                    Для подтверждения удаления введите слово <strong className="text-red-600">"УДАЛИТЬ"</strong> в поле ниже:
                                                </label>
                                                <input
                                                    type="text"
                                                    value={deleteConfirmation}
                                                    onChange={(e) => setDeleteConfirmation(e.target.value)}
                                                    placeholder="Введите УДАЛИТЬ"
                                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                                />
                                                <p className="text-sm text-gray-600 mt-1 disclaimer">
                                                    Это поле чувствительно к регистру
                                                </p>
                                            </div>

                                            <div className="flex space-x-4 pt-4 form-actions">
                                                <button
                                                    onClick={handleDeleteProfile}
                                                    disabled={isLoading || deleteConfirmation !== 'УДАЛИТЬ'}
                                                    className="px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors delete-button"
                                                >
                                                    {isLoading ? 'Удаление...' : 'Удалить профиль'}
                                                </button>
                                                <button
                                                    onClick={() => {
                                                        setActiveSection('overview');
                                                        setDeleteConfirmation('');
                                                    }}
                                                    className="px-6 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 cancel-button"
                                                >
                                                    Отмена
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            )}

                        </div>
                    </div>
                </div>
            </div>
        </div>);
}
export default SellerProfile;